    ...
    <article className="user-card" role="listitem">
      <div className="user-card__left">
        <div className="avatar-wrap">
          <NavLink to={`/user/${user.username}`} className="username-link">
      ...
      <div className="user-card__body">
        <div className="name-row">
          <NavLink to={`/user/${user.username}`} className="username-link">
            <h3 className="username" title={user.name}>{user.name}</h3>
          </NavLink>
          <span className="inline-dot" />
        </div>
        <p className="Location" title="Place">{user.location}</p>
      </div>

      <div className="user-card__right">
        <button className={`btn btn--follow ${user.followed ? "following" : ""}`} onClick={() => onToggleFollow(user.id)}>
...
export default function Follow() {
  const [items, setItems] = useState(Array.from({ length: 10 }, (_, i) => DUMMY_USER(i + 1)));
  ...
  const loadMore = () => {
    ...
    const more = Array.from({ length: 10 }, (_, i) => DUMMY_USER(nextIndex + i));
    setItems((prev) => [...prev, ...more]);
  };

  const toggleFollow = (id) => {
    setItems((prev) => prev.map((u) => (u.id === id ? { ...u, followed: !u.followed } : u)));
  };
  ...
  return (
    <div className="follow-page profile-page">
      ...
      <main>
        <div className="tab-row" role="tablist" aria-label="Filter users">
          <button
            role="tab"
        ...
        <section aria-label="Suggestions" role="list">
          {visible.map((user) => (
        ...
        <div style={{ textAlign: "center", marginTop: "1rem" }}>
          <button className="profile-primary-button" onClick={loadMore}>আরও দেখুন (10)</button>
        </div>
      </main>
    </div>
  );
