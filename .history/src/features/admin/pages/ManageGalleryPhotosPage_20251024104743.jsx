...
export default function ManageGalleryPhotosPage() {
  ...
  const [visible, setVisible] = useState(() =>
    Math.min(INITIAL_BATCH, loadStoredPhotos().length)
  );
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        loadTimerRef.current = setTimeout(() => {
          setVisible((prev) =>
            Math.min(prev + LOAD_MORE_BATCH, filtered.length)
          );
  ...
  const handleSearchChange = (event) => {
    ...
    setVisible((prev) =>
      prev
        ? Math.min(prev, photos.length)
        : Math.min(INITIAL_BATCH, photos.length)
    );
  };
