...
import { fetchAllCompanies, editCompany, deleteCompany } from "@/api/companyApi";
...
export default function ManageCompanyPage() {
  ...
  const [formState, setFormState] = useState({ banglaName: "", englishName: "" });
  ...
  const handleDelete = async (company) => {
    const confirmDelete = window.confirm(`"${company.banglaName}" কোম্পানি মুছে ফেলতে চান?`);
  ...
  const handleEditStart = (company) => {
    setEditing(company);
    setFormState({ banglaName: company.banglaName, englishName: company.englishName });
  };
            ...
            <div className="card w-100">
              ...
              <div className="card-body">
                <AdminDataTable
                  ...
                  getRowClassName={(row) => (removing[row._id] ? "is-removing" : "")}
                />
              </div>
            </div>
          </div>
