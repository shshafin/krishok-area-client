...
import {
  ...
  editCropDetails,
  deleteCropDetails,
} from "@/api/cropDetails.api"; // তোমার API ফাইলের সঠিক path বসাও
...
export default function ManageCropDetailsPage() {
  ...
  const categories = useMemo(() => {
    const list = Array.from(
      new Set(cropDetails.map((detail) => detail.category?.banglaName).filter(Boolean))
    );
    return list;
  }, [cropDetails]);
  ...
  if (loading) {
    return (
      <div className="content-wrapper _scoped_admin">
        <div className="p-5 text-center text-muted">Loading crop details...</div>
      </div>
    );
  }
