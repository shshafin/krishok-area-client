...
const loadStoredCompanies = () => {
  ...
  try {
    ...
    const normalized = parsed
      .map((item, index) => normalizeCompany(item, index))
      .filter(Boolean);
...
export default function ManageCompanyPage() {
  ...
  const handleDelete = (company) => {
    ...
    setTimeout(() => {
      ...
      toast.success(
        `"${company.banglaName}" কোম্পানি তালিকা থেকে সরানো হয়েছে।`
      );
    }, 280);
  };
  ...
  const handleFormSubmit = (event) => {
    ...
    setCompanies((prev) =>
      prev.map((item) => (item.id === editing.id ? payload : item))
    );
            ...
            <div className="card w-100">
              <div className="card-header d-flex flex-column flex-md-row gap-3 justify-content-md-between align-items-md-center">
                <h3 className="card-title mb-0">
                  Total Company Category = [{totalCompanies}]
                </h3>
                <div
