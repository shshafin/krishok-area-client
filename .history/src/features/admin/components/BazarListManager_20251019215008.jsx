...
const normalizeEntry = (entry, index) => {
  ...
  return {
    ...
    imageUrl: entry.imageUrl ?? "https://picsum.photos/seed/bazar/640/480",
    recordedAt: entry.recordedAt ?? new Date().toISOString(),
    metadata: Array.isArray(entry.metadata)
      ? entry.metadata.filter((item) => item && typeof item === "object" && item.label && item.value)
      : [],
  };
};
...
const defaultConfirmBody = (entry) => `Are you sure you want to delete entry #${entry.id}?`;
...
export default function BazarListManager({
  ...
  const [items, setItems] = useState(() => entries.map(normalizeEntry).filter(Boolean));
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const metadataFields = item.metadata.map((meta) => `${meta.label} ${meta.value}`);
  ...
  return (
    <div className="content-wrapper _scoped_admin manage-gallery-page" style={{ minHeight: "839px" }}>
        ...
        <div className="container-fluid">
          <div className="row mb-2 align-items-center">
            <div className="col-sm-6">
              ...
              {description ? <p className="text-muted mt-1 mb-0">{description}</p> : null}
            </div>
            <div className="col-sm-6">
            ...
            <div className="card-body">
              {filtered.length ? (
                <div className="manage-gallery-grid">
                  {filtered.map((entry) => (
                    <article key={entry.id} className={`manage-gallery-card ${removing[entry.id] ? "is-removing" : ""}`}>
                      <div className="manage-gallery-thumb">
                        <img src={entry.imageUrl} alt={`${entry.title || "Bazar entry"} ${entry.id}`} />
                      </div>
                      <div className="manage-gallery-body">
                        <p className="manage-gallery-description">{entry.description || "No description provided."}</p>
                      </div>
                      <footer className="manage-gallery-footer">
                        <div className="manage-gallery-info">
                          <span className="manage-gallery-time">{formatTimestamp(entry.recordedAt)}</span>
                          {entry.metadata.map(({ label, value }) => {
                            const isLocation = label.toLowerCase() === "location";
                            return (
                              <div
                                key={`${entry.id}-${label}`}
                                className={`manage-gallery-metadata${isLocation ? " manage-gallery-metadata--location" : ""}`}
                              >
                                {isLocation ? (
                                  <>
                                    <LocationPinIcon
                                      ...
                                      aria-label="Location"
                                    />
                                    <span className="manage-gallery-location-text">{value}</span>
                                  </>
                                ) : (
                                  <>
                                    <span className="manage-gallery-label">{label}</span>
                                    <span className="manage-gallery-uploader">{value}</span>
                                  </>
                                )}
                              </div>
                            );
        ...
        <div className="admin-modal-backdrop" role="presentation">
          ...
          >
            <div className="admin-modal-header">
              <h5 id="bazar-delete-title" className="mb-0">
                {confirmTitle}
              </h5>
            </div>
            <div id="bazar-delete-description" className="admin-modal-body">
              <p className="mb-2">
                {typeof confirmBody === "function" ? confirmBody(confirmEntry) : confirmBody}
              </p>
